...
# docker run --rm -ti --name gx-export --network gw-export-nw -v ${PWD}/config/php/php.ini-production:/usr/local/etc/php/php.ini -v ${PWD}/config/php/etc/apache2/apache2.conf:/etc/apache2/apache2.conf -v ${PWD}/config/php/etc/apache2/sites-available/000-default.conf:/etc/apache2/sites-available/000-default.conf -v ${PWD}/src:/var/www/html -p 9292:80 -w /var/www/html hutgate/php74-dev